...
function Sidebar({isMenuOpen, setIsMenuOpen}) {
  ...
  return (
    <div className={` top-5 left-0 flex flex-col h-screen min-w-72 p-5
dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30
border-r border-[#80609F]/30 backdrop-blur-3xl max-md:absolute
transition-all duration-500 z-50 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      {/* Logo */}
      <img src={assets.logo_full_dark} alt="brand logo" className='w-full max-w-50  ' />
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>{
        chats.filter((chat)=> chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat)=>(
          <div onClick={()=>{navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
            ...
            <img src={assets.bin_icon} className='opacity-0 group-hover:opacity-100 transition-opacity w-4 cursor-pointer not-dark:invert' alt="" />
          </div>
        ))
        }</div>
        ...
        <div onClick={()=>{navigate('/credits') ; setIsMenuOpen(false)}} className='flex items-center gap-2 p-3  mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
          ...
          <div className='flex flex-col text-sm'>
            ...
            <p className='text-xs text-gray-400'>Purchase credits to use Lumi </p>
          </div>
        ...
        {/* user account */}
         ...
         <p className='flex-1 text-sm dark:text-primary truncate'> {user ? user.name : 'login your accound'}</p>
